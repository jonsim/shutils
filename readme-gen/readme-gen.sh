...
usage()
{
    ...
    echo ""
    echo "Simple, not hugely robust, script to automatically generate the documentation"
    echo "for all shutils."
    echo ""
    echo "This is done by: for all <shutil>.examples.* files alongside this script,"
    echo "running:"
    echo "  <shutil> -h"
    echo "to retrieve its usage information and running:"
    echo "  sh <shutil>.examples.sh"
    echo "if available to generate the example information. This <shutil>.examples.sh"
    echo "script should call 'set -v' accordingly to ensure the commands are displayed"
    echo "with the output. 'set +v' can be interleaved to disable logging, e.g. to"
    echo "perform setup or tear-down actions."
    echo "If no .sh file exists, the contents of the matching file is simply appended."
    echo ""
